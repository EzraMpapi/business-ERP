                ...
                return qualData.length===0?<p className="text-slate-400 text-center py-4">No loans</p>:(
                  <div>
                    <ResponsiveContainer width="100%" height={120}>
                      <RPieChart><Pie data={qualData} dataKey="value" cx="50%" cy="50%" outerRadius={50} innerRadius={26}>
...
function CommandPalette({ isOpen, onClose, onNavigate }) {
  ...
  const dQuery    = useDebounce(query, 150);

  React.useEffect(() => {
    if (isOpen) { setTimeout(() => inputRef.current?.focus(), 50); setQuery(""); setSelected(0); setCategory("All"); }
  }, [isOpen]);
  ...
  React.useEffect(() => {
    function onKey(e) {
      ...
      if (e.key === "Enter") {
        ...
        if (item) { onNavigate(item.module, item.tab); onClose(); }
      }
    }
    window.addEventListener("keydown", onKey);
    return () => window.removeEventListener("keydown", onKey);
  }, [isOpen, selected, onClose, onNavigate]);

  const ALL_COMMANDS = [
    // Navigation
    { module:"dashboard",   tab:null,        label:"Dashboard",            desc:"Business overview & KPIs",                                 icon:"📊", cat:"Navigate" },
    { module:"crm",         tab:null,        label:"CRM — Pipeline",       desc:"Leads, customers, interactions",                           icon:"👥", cat:"Navigate" },
    { module:"sales",       tab:"invoices",  label:"Sales — Invoices",     desc:"Create and manage invoices",                               icon:"🧾", cat:"Navigate" },
    { module:"sales",       tab:"quotations",label:"Sales — Quotations",   desc:"Create quotations for clients",                            icon:"📋", cat:"Navigate" },
    { module:"inventory",   tab:null,        label:"Inventory",            desc:"Stock levels, reorder alerts",                             icon:"📦", cat:"Navigate" },
    { module:"procurement", tab:null,        label:"Procurement",          desc:"Purchase orders & suppliers",                              icon:"🚚", cat:"Navigate" },
    { module:"finance",     tab:"expenses",  label:"Finance — Expenses",   desc:"Record and track expenses",                                icon:"💸", cat:"Navigate" },
    { module:"finance",     tab:"journal",   label:"Finance — Journal",    desc:"Manual journal entries",                                   icon:"📔", cat:"Navigate" },
    { module:"finance",     tab:"reports",   label:"Finance — P&L",        desc:"Profit & loss statement",                                  icon:"📈", cat:"Navigate" },
    { module:"hr",          tab:"employees", label:"HR — Employees",       desc:"Staff list and profiles",                                  icon:"👔", cat:"Navigate" },
    { module:"hr",          tab:"payroll",   label:"HR — Payroll",         desc:"Run monthly payroll",                                      icon:"💰", cat:"Navigate" },
    { module:"hr",          tab:"leave",     label:"HR — Leave",           desc:"Leave requests and approvals",                             icon:"🌴", cat:"Navigate" },
    { module:"healthcare",  tab:"patients",  label:"Healthcare — Patients",desc:"Patient registry & MRN",                                   icon:"🏥", cat:"Navigate" },
    { module:"healthcare",  tab:"doctorreview",label:"Doctor Review",      desc:"Clinical review & prescriptions",                          icon:"🩺", cat:"Navigate" },
    { module:"healthcare",  tab:"laboratory",label:"Laboratory",           desc:"Lab test orders & results",                                icon:"🧪", cat:"Navigate" },
    { module:"pharmacy",    tab:"stock",     label:"Pharmacy — Stock",     desc:"Drug inventory & expiry",                                  icon:"💊", cat:"Navigate" },
    { module:"pharmacy",    tab:"expiry",    label:"Pharmacy — Expiry",    desc:"Check drugs expiring soon",                                icon:"⚠️", cat:"Navigate" },
    { module:"banking",     tab:"teller",    label:"Banking — Teller",     desc:"Cash deposits & withdrawals",                              icon:"🏦", cat:"Navigate" },
    { module:"banking",     tab:"loans",     label:"Banking — Loans",      desc:"Loan portfolio management",                                icon:"📜", cat:"Navigate" },
    { module:"restaurant",  tab:"order",     label:"Restaurant — Order",   desc:"Take table orders",                                        icon:"🍽️", cat:"Navigate" },
    { module:"restaurant",  tab:"kitchen",   label:"Kitchen Display",      desc:"Active orders & kitchen queue",                            icon:"👨‍🍳", cat:"Navigate" },
    { module:"hotel",       tab:"checkin",   label:"Hotel — Check-In",     desc:"Check in a new guest",                                     icon:"🏨", cat:"Navigate" },
    { module:"school",      tab:"students",  label:"School — Students",    desc:"Student enrolment & registry",                             icon:"🎓", cat:"Navigate" },
    { module:"school",      tab:"fees",      label:"School — Fee Collection",desc:"Collect student fees",                                   icon:"💳", cat:"Navigate" },
    { module:"microfinance",tab:"loans",     label:"Microfinance — Loans", desc:"MFI loan portfolio & PAR",                                 icon:"🏧", cat:"Navigate" },
    { module:"vicoba",      tab:"members",   label:"VICOBA Members",       desc:"Group savings members",                                    icon:"🤝", cat:"Navigate" },
    { module:"fleet",       tab:"vehicles",  label:"Fleet — Vehicles",     desc:"Vehicle status & maintenance",                             icon:"🚗", cat:"Navigate" },
    { module:"projects",    tab:null,        label:"Projects",             desc:"Project tracker & milestones",                             icon:"🏗️", cat:"Navigate" },
    { module:"analytics",   tab:null,        label:"Analytics",            desc:"Business intelligence charts",                             icon:"📉", cat:"Navigate" },
    { module:"reports",     tab:null,        label:"Reports",              desc:"Financial statements",                                     icon:"📑", cat:"Navigate" },
    { module:"settings",    tab:null,        label:"Settings",             desc:"Company setup & preferences",                              icon:"⚙️", cat:"Navigate" },
    // Actions
    { module:"sales",       tab:"invoices",  label:"New Invoice",          desc:"Create a new customer invoice",                            icon:"➕", cat:"Action", action:"new" },
    { module:"hr",          tab:"employees", label:"Add Employee",         desc:"Register a new employee",                                  icon:"➕", cat:"Action", action:"new" },
    { module:"inventory",   tab:null,        label:"Add Stock Item",       desc:"Add new item to inventory",                                icon:"➕", cat:"Action", action:"new" },
    { module:"healthcare",  tab:"patients",  label:"Register Patient",     desc:"Add new patient to system",                                icon:"➕", cat:"Action", action:"new" },
    { module:"pharmacy",    tab:"dispensing",label:"Dispense Drug",        desc:"Issue prescription medication",                            icon:"➕", cat:"Action", action:"new" },
    { module:"banking",     tab:"teller",    label:"Cash Deposit",         desc:"Process a cash deposit",                                   icon:"➕", cat:"Action", action:"deposit" },
    { module:"banking",     tab:"teller",    label:"Cash Withdrawal",      desc:"Process a cash withdrawal",                                icon:"➕", cat:"Action", action:"withdraw" },
    { module:"restaurant",  tab:"order",     label:"New Table Order",      desc:"Start a new restaurant order",                             icon:"➕", cat:"Action", action:"new" },
    { module:"hotel",       tab:"checkin",   label:"Check-In Guest",       desc:"Check in a hotel guest",                                   icon:"➕", cat:"Action", action:"new" },
    { module:"school",      tab:"students",  label:"Enroll Student",       desc:"Register a new student",                                   icon:"➕", cat:"Action", action:"new" },
  ];

  const CATEGORIES = ["All", "Navigate", "Action"];

  const filtered = React.useMemo(() => {
    let items = category === "All" ? ALL_COMMANDS : ALL_COMMANDS.filter(c => c.cat === category);
    if (!dQuery) return items.slice(0, 12);
  ...
  React.useEffect(() => { setSelected(0); }, [dQuery, category]);
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-start justify-center pt-[10vh]"
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-xl mx-4 overflow-hidden border border-slate-200"
           style={{boxShadow:"0 25px 50px rgba(0,0,0,.25)"}}
        ...
        <div className="flex items-center gap-3 px-4 py-3.5 border-b border-slate-100">
          ...
          <input
            ...
            className="flex-1 text-[14.5px] text-slate-800 bg-transparent outline-none placeholder:text-slate-400"
          />
          {query && <button onClick={()=>setQuery("")} className="text-slate-400 hover:text-slate-600"><X size={14}/></button>}
          <kbd className="text-[10px] bg-slate-100 text-slate-400 px-2 py-1 rounded-md font-mono shrink-0">ESC</kbd>
        </div>

        {/* Category tabs */}
        <div className="flex gap-1 px-3 pt-2 pb-1 border-b border-slate-50">
          {CATEGORIES.map(cat=>(
            <button key={cat} onClick={()=>setCategory(cat)}
              className={"px-3 py-1 rounded-lg text-[12px] font-medium transition-all "+(category===cat?"bg-[#16A34A] text-white":"text-slate-400 hover:text-slate-600 hover:bg-slate-50")}>
              {cat}
            </button>
          ))}
          <div className="ml-auto text-[11px] text-slate-300 self-center">{filtered.length} result{filtered.length!==1?"s":""}</div>
        </div>
        ...
        <div className="max-h-72 overflow-y-auto py-1">
          {filtered.length === 0 ? (
            ...
            <div key={item.label+i}
              onClick={()=>{ onNavigate(item.module, item.tab); onClose(); }}
              onMouseEnter={()=>setSelected(i)}
              className={"flex items-center gap-3 px-4 py-2.5 cursor-pointer transition-colors "+(selected===i?"bg-[#16A34A]":"hover:bg-slate-50")}>
              <span className="text-[18px] shrink-0 w-7 text-center">{item.icon}</span>
              <div className="flex-1 min-w-0">
                <p className={"text-[13.5px] font-semibold truncate "+(selected===i?"text-white":"text-[#111827]")}>{item.label}</p>
                <p className={"text-[11.5px] truncate "+(selected===i?"text-green-100":"text-slate-400")}>{item.desc}</p>
              </div>
              {item.cat==="Action" && (
                <span className={"text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 "+(selected===i?"bg-white/20 text-white":"bg-green-50 text-green-700")}>ACTION</span>
              )}
              {selected===i && <kbd className="text-[10px] bg-white/20 text-white px-1.5 py-0.5 rounded font-mono shrink-0">↵</kbd>}
            </div>
          ))}
        </div>

        {/* Footer */}
        <div className="border-t border-slate-100 px-4 py-2 flex items-center justify-between text-[10.5px] text-slate-400 bg-slate-50">
          <div className="flex items-center gap-3">
            <span className="flex items-center gap-1"><kbd className="bg-white border border-slate-200 px-1.5 py-0.5 rounded font-mono">↑↓</kbd> navigate</span>
            <span className="flex items-center gap-1"><kbd className="bg-white border border-slate-200 px-1.5 py-0.5 rounded font-mono">↵</kbd> open</span>
            <span className="flex items-center gap-1"><kbd className="bg-white border border-slate-200 px-1.5 py-0.5 rounded font-mono">esc</kbd> close</span>
          </div>
          <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-green-400"/>{ALL_COMMANDS.length} commands available</span>
        </div>
      </div>
    </div>
  );
...
// category is printed on each card — cybersecurity reads THIS device
...
// stock exposure because per-item supplier links do not exist yet.
function RiskCenterView({ invoices, expenses, inventory, employees }) {
...
// to Business Intelligence Center, since that is what this module actually
...
function Analytics({ company, invoices, expenses, crm, inventory, employees, leaveRequests, workOrders, posTransactions, onNavigate }) {
...
function BreakdownBars({ items, formatValue }) {
...
function ExecutiveDashboard({ company, invoices, expenses, crm, inventory, employees, onNavigate }) {
...
function KpiFormPanel({ onClose, onSubmit }) {
...
function HeatMaps({ invoices, inventory }) {
...
function BenchmarkFormPanel({ onClose, onSubmit }) {
...
function CompetitorFormPanel({ onClose, onSubmit }) {
...
// this company own live data, with the exact method shown next to every
...
function PredictiveIntelligence({ invoices, expenses, inventory, employees, leaveRequests }) {
  ...
  // (this company own trailing 8-week average expense spend, not a
  ...
  // 8. Maintenance Needs — directly real: every machine own most recent
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-start gap-2.5 bg-slate-50 border border-slate-100 rounded-lg p-3">
        ...
        <p className="text-[12px] text-slate-500 leading-relaxed">
          These are real statistical projections and rule-based risk heuristics computed from this company own live data — not a trained machine-learning model, since no dataset here is large enough to train one on and no infrastructure exists to serve one. Every number below shows its own method; none of it is an unexplained confidence score.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5">
          ...
          <p className="text-[11px] text-slate-400 mb-3">Real burn rate (spend ÷ time elapsed) projected across each project full real timeline</p>
        ...
        <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5">
          ...
          <p className="text-[11px] text-slate-400 mb-3">Directly real — each machine own logged next-due date, not a modeled estimate</p>
...
// transparent arithmetic on this business own real baseline numbers —
...
function ScenarioPlanner({ invoices, expenses, employees }) {
  ...
  const result = useMemo(() => {
    ...
    if (scenario === "hiring") {
      ...
      return {
        ...
        caveat: baseline.employeeCount === 0 ? "No active employees on record yet — this uses a real average of zero, which won't be meaningful until real payroll data exists." : "Uses this company own real average salary, not an assumed figure — a specialized role could cost meaningfully more or less than this average.",
      };
    }
    ...
    return {
      ...
      caveat: "Assumes the cut does not affect revenue or operations — cutting some categories (marketing, maintenance) can have real knock-on effects this simple model does not capture.",
    };
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4 sm:p-5">
      ...
      <p className="text-[12px] text-slate-500 mb-4">Real arithmetic on this business own real numbers — not a prediction, a calculator. Model a decision before making it.</p>
        ...
        <div>
          ...
          {scenario === "hiring" && (
            <div>
              ...
              <p className="text-[11px] text-slate-400 mt-1.5">Uses this company own real average salary across {baseline.employeeCount} active employees.</p>
            </div>
          )}
          {scenario === "expense" && (
            <div className="space-y-3">
...
async function executeWorkflowStep(step, context) {
  ...
  if (step.type === "notify_slack" || step.type === "notify_teams") {
    ...
    if (!channel?.enabled || !channel?.webhookUrl) return { ok: false, note: `${stepType.label.replace("Notify via ", "")} is not configured — set it up in Notifications > Channels first.` };
...
function WorkflowStudio({ company, invoices, expenses, inventory }) {
  ...
  // Install — copies a template's real steps into this company own
  ...
  // Publish — a company own workflow becomes a public template. Email
  ...
  async function runWorkflow(workflow) {
    ...
    if (IS_CONFIGURED && workflow.dbId) {
      try { await sb("workflows").eq("id", workflow.dbId).update({ last_run: today }).run(); } catch (_e) { /* the run itself already completed; a log-sync miss is not worth a second toast */ }
    }
  }
...
export default MicrofinanceModule;
