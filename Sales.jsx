...
function Sales({ invoices, inventory, subscriptionsHook, quotationsHook, currentUser, intent, clearIntent }) {
  ...
  // Subscriptions is not a document tab — it's handled by its own
      ...
      "<div class='page' style='position:relative'>" +
        ...
        "<div class='header'>" +
          "<div class='company-block'>" +
            logoHtml +
            "<div>" +
              "<div class='company-name'>" + (co.name||"BusinessSphere") + "</div>" +
        ...
        : "<div style='padding:16px 40px 20px;border-top:1px solid #F3F4F6'>" +
            "<div style='font-size:11.5px;color:#9CA3AF;text-align:center;font-style:italic'>" +
              "Please transfer payment to the account details provided by " + (co.name||"BusinessSphere") + " · Quote invoice number " + doc.id + " as your reference." +
            "</div></div>") +
        ...
        // Footer
        "<div class='footer'>" +
          "<div class='footer-note'>Thank you for your business · " + (co.name||"BusinessSphere") + "</div>" +
          "<div style='display:flex;align-items:center;gap:12px'>" +
            qrSvg +
            "<div class='footer-brand'>BusinessSphere ERP</div>" +
          "</div>" +
        "</div>" +
  ...
  function convertQuoteToInvoice(quote) {
    ...
    const newInv = {
      ...
      customer: quote.customer, date: TODAY_STR,
                ...
                <div className="bg-white rounded-xl border border-slate-200/80 shadow-sm p-4">
                  ...
                  <div className="flex items-center gap-4">
                    <ResponsiveContainer width="55%" height={140}>
                      <RPieChart><Pie data={statusData} dataKey="value" cx="50%" cy="50%" outerRadius={58} innerRadius={30}>
                        {statusData.map((d,i)=><Cell key={i} fill={d.fill}/>)}
                      </Pie><Tooltip formatter={(v,n)=>[v+" invoices",n]}/></RPieChart>
                    </ResponsiveContainer>
...
_${co.name||"BusinessSphere"}_`;
              ...
              <button
                onClick={() => {
                  const co = window.__smartManagerCompany||{};
                  const total = lineTotal(doc.items||[]).total;
                  const subj  = encodeURIComponent(`Invoice ${doc.id} from ${co.name||"BusinessSphere"}`);
...
function OrderReturnFormPanel({ order, onClose, onSubmit }) {
...
function DocFormPanel({ kind, onClose, onSubmit, inventory }) {
  const meta = {
...
function Subscriptions({ subscriptions, invoices }) {
  const [query, setQuery] = useState("");
  const [selected, setSelected] = useState(null);
  const [showForm, setShowForm] = useState(false);
      ...
      <form onSubmit={handleSubmit} className="relative w-full sm:w-[400px] bg-white h-full shadow-2xl overflow-y-auto flex flex-col" style={{ animation: "slideIn .15s ease-out" }}>
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          ...
          <p className="text-[11.5px] text-slate-400">
            The first invoice is not generated automatically — open the subscription and click Generate Invoice when the cycle is due.
          </p>
        </div>
...
function InventoryDashboard({ inventory, suppliersHook }) {
