...
function WorkflowBuilder({ workflow, onClose, onSave }) {
    ...
    <div className="fixed inset-0 z-40 bg-white flex flex-col">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-64 border-r border-slate-100 p-4 overflow-y-auto shrink-0 hidden sm:block">
          ...
          <p className="text-[11px] text-slate-400 mt-4 leading-relaxed">Or tap a step below to add it on mobile, where dragging is not available the same way.</p>
        ...
        <div className="px-6 py-5 flex-1 space-y-4">
          ...
          {hasEmailStep && (
            <div className="flex items-start gap-2 bg-[#F59E0B]/10 rounded-lg p-3">
              ...
              <p className="text-[11.5px] text-[#8a670a] leading-relaxed">This workflow includes an email step with a recipient address — that is cleared automatically before publishing, since it's your own contact data, not something that belongs in a shared template.</p>
            </div>
          )}
...
function ResourceSchedulerPanel({ currentUser }) {
...
function VideoMeetingBar({ currentUser }) {
...
/* ═══════════════════════════════════════════════════════════════════════
   WHATSAPP CENTER — In-system WhatsApp messaging hub
   ─ Contact list built from CRM leads + employees
...
const waBus = { listeners: new Set(), push(payload) { this.listeners.forEach(fn=>fn(payload)); } };

function useWaMessages(contactId) {
...
function WhatsAppCenter({ currentUser, crm, employees, invoices, company }) {
  ...
  function applyTemplate(t) {
    ...
    const merged = t.body
      .replace(/\{company\}/g, co.name||"BusinessSphere")
...
/* ═══════════════════════════════════════════════════════════════════════
   EMAIL CENTER — In-system email client
   ─ Inbox / Sent / Drafts / Starred folders
   ─ Full email composer with To/CC/BCC/Subject/Body
   ─ Business email templates (invoice, reminder, statement, welcome…)
   ─ Send via mailto: (client email app) OR SMTP config (custom backend)
   ─ Email history stored in Supabase (emails table)
   ─ Contact picker from CRM + employees
   ─ Quick-compose from any module (emailBus)
═══════════════════════════════════════════════════════════════════════ */
...
// the modules it's reading from — there is only one copy of the truth.
...
function computeTaxDeadlines(monthsAhead = 3) {
  const deadlines = [];
...
function SharedCalendar({ invoices, crm, workOrders, leaveRequests }) {
...
// File Sharing does not duplicate the Document Center (section 36) — it
...
function NotebookView({ currentUser }) {
...
// body in this mode, so a call that does not throw means "the request left
// the browser," not "Slack confirmed receipt." That distinction matters
// and the UI says so explicitly after every test send.
async function sendWebhookNotification(url, text) {
